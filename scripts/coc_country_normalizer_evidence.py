#!/usr/bin/env python3
"""Evaluate the CoC-country shadow-normalizer evidence corpus."""
...
from query_understanding.compound_prompt_normalizer_evidence import (
    ...
    evaluate_coc_country_llm_corpus,
    load_corpus,
    write_report,
)
...
DEFAULT_CORPUS = Path("docs/eval-evidence/coc-country-shadow-normalizer-corpus-2026-07-01.json")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate the CoC-country shadow-normalizer evidence corpus.")
    parser.add_argument("--corpus", default=str(DEFAULT_CORPUS), help="Path to the CoC-country evidence corpus JSON.")
    ...
    if args.invoke_llm:
        ...
        report = evaluate_coc_country_llm_corpus(load_corpus(Path(args.corpus)), provider=provider)
    else:
        report = evaluate_coc_country_evidence_corpus(load_corpus(Path(args.corpus)))
    if args.output:
