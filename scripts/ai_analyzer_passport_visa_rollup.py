...
from ai_analyzer_passport_visa_folder_diagnostic import _build_analyzer, _build_row


REPO_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_RESUME_ROOT = Path("/Users/kartikraghavan/Library/Application Support/NjordHR/Resumes")
DEFAULT_OUTPUT = REPO_ROOT / "AI_Search_Results" / "ai_analyzer_passport_visa_rollup_2026-05-07.json"
...
def _collect_email_examples(rows, limit=5):
    ...
    email_rows.sort(
        key=lambda row: (
            0 if (row.get("resolved_logistics") or {}).get("passport_expiry_date") is None else 1,
            0 if (row.get("visa_fact") or {}).get("visa_records") else 1,
            row["filename"].lower(),
        )
    )
    return [
        {
            ...
            "passport_status": (row.get("resolved_logistics") or {}).get("passport_expiry_status"),
            "passport_expiry_date": (row.get("resolved_logistics") or {}).get("passport_expiry_date"),
            "visa_status": (row.get("visa_fact") or {}).get("status"),
            "visa_records": (row.get("visa_fact") or {}).get("visa_records"),
...
def main():
    parser = argparse.ArgumentParser(description="Horizontal passport/visa rollup across live rank folders.")
    ...
    output = {
        ...
        "scope_note": (
            "Horizontal passport/visa validation pass across live rank folders. "
            "This splits counts by EMAIL_ filename pattern versus non-email PDFs."
        ),
