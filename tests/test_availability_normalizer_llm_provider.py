...
class AvailabilityNormalizerLlmProviderTests(unittest.TestCase):
    ...
    def test_coc_country_prompt_uses_catalog_without_executor_ids(self):
        ...
        self.assertIn("USA-issued CoC -> countries=[\"usa\"], operator=\"contains_any\"", prompt)
        self.assertIn("Only show resumes that mention Indian CoC", prompt)
        self.assertIn("operator=\"contains_any\", display_value=\"Indian CoC\"", prompt)
        self.assertIn("display_value=\"exactly USA CoC only\"", prompt)
        self.assertIn("Do not shorten this phrase to \"USA CoC\"", prompt)
        self.assertIn("display_value=\"strictly Indian CoC only\"", prompt)
        self.assertIn("Do not shorten this phrase to \"Indian CoC\"", prompt)
        self.assertIn("display_value=\"only UK CoC\"", prompt)
        self.assertIn("Do not shorten this phrase to \"UK CoC\"", prompt)
