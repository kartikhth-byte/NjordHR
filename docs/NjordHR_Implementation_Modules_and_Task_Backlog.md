...
- `P1` CoC country compound-normalizer rollout is in fixture-evidence stage: `coc_country_match` is declared in `candidate_facts/aliases/filter_capability_catalog.json` using canonical country IDs from `candidate_facts/aliases/coc_country.json`, and its provider prompt plus 200-prompt fixture corpus live in `docs/eval-evidence/coc-country-shadow-normalizer-corpus-2026-07-01.json` with Class A=80, Class B=80, Class C=40 and required cross-family coverage against both promoted families (`availability`, `vessel_tonnage`). It remains unpromoted, so `PROMOTED_FAMILIES` stays `{"availability", "vessel_tonnage"}` and live dispatch remains unchanged.
