...
- `P1` Availability filter rollout contract is locked in `docs/specs/availability_filter_v1.md`; deterministic extraction, picker semantics, and Needs Review behavior are implemented, the audit-only `availability` capability catalog row is added, the 200-prompt shadow-normalizer evidence corpus is tracked in `docs/eval-evidence/availability-shadow-normalizer-corpus-2026-06-29.json`, and the first real Gemini audit artifact is tracked in `docs/eval-evidence/availability-shadow-normalizer-llm-evidence-2026-06-30.json`. The evidence gate passes in the artifact; live promotion remains pending an explicit promotion/cutover PR with deterministic enforcement coverage.
