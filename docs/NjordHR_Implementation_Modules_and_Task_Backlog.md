...
- `P1` CoC country compound-normalizer rollout is started with a catalog-only slice: `coc_country_match` is declared in `candidate_facts/aliases/filter_capability_catalog.json` using canonical country IDs from `candidate_facts/aliases/coc_country.json`; it remains unpromoted, so `PROMOTED_FAMILIES` stays `{"availability", "vessel_tonnage"}` and live dispatch remains unchanged.
