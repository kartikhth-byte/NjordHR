...
- `P1` Compound-normalizer dispatch-strategy telemetry is implemented for AI Search completion events: `/analyze` and `/analyze_stream` system-log telemetry include `compound_normalizer_mode` and `compound_normalizer_dispatch_strategy`, while client payloads, SSE events, CSV columns, durable audit-event fields, provider dispatch behavior, and `PROMOTED_FAMILIES` remain unchanged.
- `P1` Age-range compound-normalizer backfill has started: `age_range` is declared in `candidate_facts/aliases/filter_capability_catalog.json` with minimum/maximum year bounds, plausibility range `[16, 75]`, validator coverage, provider prompt coverage, and a 200-prompt fixture corpus in `docs/eval-evidence/age-range-shadow-normalizer-corpus-2026-07-01.json` with Class A=80, Class B=80, Class C=40 and required cross-family `availability`, `vessel_tonnage`, and `coc_country_match` prompts across Class A, Class B, and Class C. The existing deterministic parser and `LLM_Promotion_Stage` rescue path remain authoritative until a later real-Gemini evidence and cutover PR promotes `age_range` into `PROMOTED_FAMILIES`.
